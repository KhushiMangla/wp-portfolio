...
function fwd_scripts() {
	wp_enqueue_style( 
		'fwd-googlefonts', 
		"https://fonts.googleapis.com/css2?family=Noto+Serif:wght@400;700&family=Open+Sans:wght@400;700&display=swap",
		array(),// dependencies
	...
	wp_enqueue_script( 'fwd-scroll-top', get_template_directory_uri() . '/js/scroll-top.js', array(), _S_VERSION,  array( 'strategy' => 'defer' ) );
...
// Add theme Color meta tag and action hook
...
// Create Block templates for contact page
function fwd_block_editor_templates() {
    ...
    if ( isset( $_GET['post'] ) && '76' == $_GET['post'] ) {
        $post_type_object = get_post_type_object( 'page' );
        $post_type_object->template = array(
			array( 
				'core/paragraph', 
...
function fwd_post_filter_home( $use_block_editor, $post ) {
    // Change 112 to your Page ID
    $page_ids = array( 12 );
    if ( in_array( $post->ID, $page_ids ) ) {
        return false;
...
add_filter( 'use_block_editor_for_post', 'fwd_post_filter_home', 10, 2 );
